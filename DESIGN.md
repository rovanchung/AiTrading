...
- **manager.py** — Three-step decision logic (parameters are version-dependent, see accounts config):
  - **Step 1 — Profit-based sells**: Sell if P&L ≥ profit target or ≤ -loss cut from Alpaca avg cost. V2-strategy accounts respect min hold time. Sold tickers enter a `cooldown_hours` window before they can be re-bought. The cooldown is enforced not just at signal generation (redistribution skips cooled-down names) but also on the two position re-open paths: a limit buy that fills *after* the name was exited (`_reconcile_pending_buys`) and an untracked Alpaca holding picked up by drift reconcile (`reconcile_positions`) are both flattened instead of re-tracked (`_flatten_cooldown_fill`). Without this, a limit buy resting from before the exit could fill post-cooldown and spin a loss-cut → rebuy loop.
