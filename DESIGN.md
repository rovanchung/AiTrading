...
- **manager.py** — Three-step decision logic (parameters are version-dependent, see accounts config):
  - **Step 1 — Profit-based sells**: Sell if P&L ≥ profit target or ≤ -loss cut from Alpaca avg cost. V2-strategy accounts respect min hold time. Sold tickers enter a `cooldown_hours` window before they can be re-bought. The cooldown is enforced not just at signal generation (redistribution skips cooled-down names) but also on the position re-open paths: a buy that fills *after* the name was exited (`_reconcile_pending_buys`) and an untracked Alpaca holding picked up by drift reconcile (`reconcile_positions` / `sync_local_state`) are flattened instead of re-tracked (`_flatten_cooldown_fill`). Separately, those reconcile paths skip any name whose own exit sell is **still in flight** (Alpaca shows the shares until the market sell fills): re-tracking it would reset `entry_time` and re-arm the loss-cut, which is what produced runs of same-entry/same-exit phantom loss-cuts. Exit sells are market orders and are never cancelled, so they complete instead of churning.
  - **Step 2 — Score-based sells (`no_longer_qualifies`)**: Held positions whose composite score has fallen below `v2_sell_threshold` are sold. By default a single sub-threshold score triggers the exit. Two optional debounce rules can be configured (OR semantics — either one firing triggers the sell): `v2_no_longer_qualifies_consecutive: N` requires N consecutive sub-threshold scores, and `v2_no_longer_qualifies_ma_window: M` requires the moving average over the last M scores to be sub-threshold. Both default to `0` (disabled). When a position is sub-threshold but the debounce condition is not yet met, it is held as-is and **excluded from step 3** for the cycle so redistribute neither buys more nor partially sells it down. Running this step before redistribution prevents the buy-then-immediate-sell pattern seen when a name's score flaps across the threshold each minute.
  - **Step 3 — Score-based redistribution**: Allocates 50% of portfolio value (`purchase_power_pct`) across all qualifying stocks. The qualifying set is: candidates with composite ≥ `buy_threshold` (macro-adjusted), plus currently-held positions still scoring ≥ `v2_sell_threshold` (so held positions in the hysteresis band participate in the budget instead of free-riding outside it). Each stock gets capital proportional to its composite score share. Trades are sized as `target_qty − current_qty`; a trade is skipped if `|target$ − current$| / price ≤ v2_rebalance_min_share_diff` (per-stock share-count floor that replaces the prior equity-relative dead band). Buys are additionally skipped if today's price already trips the prior-close loss-cut / profit-take threshold (would otherwise immediately exit via step 1 next cycle).
...
- **alpaca_client.py** — Wraps `alpaca-py` SDK. Market orders for both sides: buys via `submit_market_order`, sells via `close_position` (cannot open a short). `submit_limit_order` remains available but is unused by the engine. Market clock check.
